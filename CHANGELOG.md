...
### Fixed
- **CloudFormation stack updates no longer leave duplicate API Gateway v2 routes and integrations behind.** When the same CFN stack was deployed twice (the `cdk deploy` pattern), every `AWS::ApiGatewayV2::Route` and `AWS::ApiGatewayV2::Integration` resource in the template ended up duplicated on every redeploy — after a few cycles a real-world API could be carrying hundreds of routes with the same route key, each pointing at a different copy of the integration. The cause was in the CloudFormation engine, not the API Gateway service itself: ministack's CFN engine falls back to calling the resource's `create` handler when no explicit `update` handler is registered, on the assumption that creates are idempotent. The apigw v2 Route and Integration provisioners weren't — they minted a fresh `RouteId` / `IntegrationId` via `new_uuid()` on every call and inserted under that ID, leaving the previous resource in place. Both resource types now have proper `update` handlers that look up the existing resource by the previous physical ID and mutate it in place, so a redeploy mutates the same row instead of leaving a stale copy behind. The known `AWS::ApiGatewayV2::Api` replacement quirk (documented separately) is unchanged.
