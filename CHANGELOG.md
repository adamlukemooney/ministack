...
### Fixed
- **HttpApi v2 routes with `AuthorizationType=CUSTOM` now actually invoke the configured Lambda authorizer.** The route record was correct — `get-route` reported `AuthorizationType=CUSTOM` with the right `AuthorizerId` — but the request-time dispatch only branched on `JWT` and silently fell through to the integration for every other auth type. Handlers that read `event.requestContext.authorizer.lambda` to look up caller identity returned HTTP 500; handlers that didn't served unauthenticated traffic; and the authorizer Lambda's CloudWatch log group never even materialised because the function was never invoked. The dispatcher now invokes the authorizer Lambda before the integration, supports both v2 response shapes (`EnableSimpleResponses=true` returning `{isAuthorized, context}`, and the IAM-policy shape with `principalId` + `policyDocument` + `context`), forwards the authorizer's `context` to the integration as `requestContext.authorizer.lambda`, and caches results by the resolved `IdentitySource` values for `AuthorizerResultTtlInSeconds` so repeat callers don't pay the full Lambda cold-start cost on every request. Missing identity-source values short-circuit to 401 without invoking the authorizer, matching real AWS.
- **CloudFormation `AWS::ApiGatewayV2::Stage` now round-trips `AccessLogSettings`, `DefaultRouteSettings`, and `RouteSettings` through to `get-stage`.** Three separate gaps fixed together: `AccessLogSettings` was dropped entirely (the provisioner never read the property), `DefaultRouteSettings` and per-route `RouteSettings` inner objects were stored PascalCase from the template (`DataTraceEnabled`, `DetailedMetricsEnabled`, `LoggingLevel`, `ThrottlingBurstLimit`, `ThrottlingRateLimit`), and boto3's `GetStage` response model — which expects all-camelCase — silently deserialised those fields to empty defaults. Net effect: HttpApi access logs went nowhere despite a correct template, throttling / metrics overrides on individual routes didn't apply, and there was no error to point at. The handler now camelCases the inner keys on store, the same way Round 4 fixed `CorsConfiguration` on `Api`. `ClientCertificateId` and `DeploymentId` also round-trip now (they were silently dropped before).
- **CloudFormation `AWS::ApiGatewayV2::Stage` redeploys no longer wipe the stage record.** The create handler unconditionally wrote `_stages[api_id][stage_name] = {...}`, so the engine's update-falls-back-to-create path zeroed the stage on every redeploy — anything set via the runtime `UpdateStage` since the last deploy was lost, and any field the new template happened not to mention (e.g. an alarm-configured throttling override) reverted to default. Same destructive pattern as the SNS topic and ApiGatewayV2 Api bugs fixed earlier on this branch; the create handler now short-circuits to in-place mutation when the stage already exists.
- **CloudFormation `AWS::ApiGatewayV2::Integration` now provisions `TlsConfig` and five previously-dropped flat fields.** The provisioner ignored `TlsConfig`, `CredentialsArn`, `IntegrationSubtype`, `PassthroughBehavior`, `TemplateSelectionExpression`, and `IntegrationResponseSelectionExpression`. The first costs SNI validation for private HTTPS backend integrations and is the highest-impact omission; the rest break `AWS_PROXY` service integrations (Step Functions / SQS / EventBridge subtype dispatch), cross-account integrations that need role assumption via `CredentialsArn`, and content-type passthrough behaviour. `TlsConfig` is also camelCase-normalised on store (`ServerNameToVerify` → `serverNameToVerify`) so `get-integration` deserialises it correctly.
- **CloudFormation `AWS::ApiGatewayV2::Api` updates no longer silently blank out `CorsConfiguration`.** The create and update handlers stored the CFN-shape inner keys verbatim (`AllowOrigins`, `AllowMethods`, `AllowHeaders`, …), but boto3's `get-api` response model expects camelCase (`allowOrigins`, `allowMethods`, `allowHeaders`, …) so the SDK's case-mapping deserialiser quietly dropped every field — every browser preflight against an HTTP API failed with no CORS headers until a manual `apigatewayv2 update-api --cors-configuration ...` re-applied the same config the template already declared. Same root cause as the `JwtConfiguration` case mismatch fixed earlier; the new `_apigw_v2_cors_config_camel` helper normalises inner keys to camelCase on store so the round-trip stays accurate across CREATE and UPDATE, and clearing CORS by dropping the property from a redeploy still works.
- **CloudFormation `AWS::ApiGatewayV2::Route` now propagates `AuthorizerId` and `AuthorizationScopes` to the route record.** The route create and update handlers read `AuthorizationType` but never picked up the matching `AuthorizerId` (or `AuthorizationScopes` for JWT scope-checks), so any CDK HTTP API that templated `authorizer: HttpUserPoolAuthorizer` / `HttpJwtAuthorizer` and `authorizationScopes: ["…"]` ended up with `AuthorizationType=CUSTOM`/`JWT` set on the route but no authorizer reference — the runtime couldn't resolve which authorizer to invoke, so the route returned HTTP 500 on every call even though the corresponding `AWS::ApiGatewayV2::Authorizer` resource (added in the previous round) was correctly provisioned alongside it. The handlers now mirror what the boto3 `create-route` / `update-route` paths already do: presence in props is authoritative, dropping the property on a later deploy clears the stale fields so a route that's moved back to `AuthorizationType=NONE` doesn't keep pointing at an unreferenced authorizer.
- **CloudFormation `AWS::SNS::Subscription` no longer reports `CREATE_COMPLETE` for subscriptions that were never actually created.** The previous handler had a silent-fallback branch: if the topic ARN wasn't found in the in-memory SNS store at the moment the subscription's create handler ran, it would fabricate a plausible-looking `<topic-arn>:<uuid>` ARN and return that to CFN as if the subscribe had succeeded. CFN dutifully recorded the resource as `CREATE_COMPLETE` with that fake ARN as `PhysicalResourceId`, and from then on every observability surface — `describe-stack-resources`, drift detection, stack events — reported the subscription as healthy while SNS itself held no record of it and every published message went nowhere. The most common trigger was a cross-stack race during a `cdk deploy --all` bringup: the subscription's `Fn::ImportValue` would resolve the topic ARN correctly, but the topic's in-memory record in the SNS service wouldn't yet be visible to the subscription stack's resource handler. Symptoms: 9 out of 10 sibling subscriptions on the same FIFO topic created successfully; the 10th came back as a phantom and the publisher's messages were silently dropped, requiring a manual `sns subscribe` post-deploy workaround. The CFN provisioner now routes through the same subscribe path the boto3 `sns:Subscribe` action uses — missing topic surfaces as `CREATE_FAILED` (matching real CFN), the `(protocol, endpoint)` idempotency check fires (matching real AWS Subscribe semantics, so a stack redeploy returns the existing subscription ARN instead of accumulating duplicate rows), the topic's `SubscriptionsConfirmed` count stays accurate, and `DeliveryPolicy` / `RedrivePolicy` / `SubscriptionRoleArn` round-trip correctly.
- **CloudFormation now waits for sibling stacks to publish their exports before resolving `Fn::ImportValue`.** Before, two stacks created in quick succession ran as independent asyncio tasks with no cross-stack ordering — the engine had no notion of "stack B depends on stack A's exports", so whether B's resources saw A's already-provisioned state or raced it was purely a function of how the asyncio scheduler interleaved the two tasks. Whenever B lost the race, downstream service-store lookups (SNS topic by ARN, SQS queue by URL, Lambda by name, …) silently missed for resources that the resolver had nonetheless resolved against the export name. The new pre-deploy pass scans the template for `Fn::ImportValue` literals, identifies which export names are still pending publication by an `*_IN_PROGRESS` sibling stack, and awaits those siblings to leave the in-progress state (capped at 15s) before starting the per-resource loop. Imports that target an export no in-flight stack is going to publish still fail fast with the canonical "Export not found" message — there's no penalty for intentionally-bad templates.
- **CloudFormation `AWS::SNS::Topic` redeploys no longer wipe the topic's subscription list.** The topic provisioner unconditionally overwrote `_sns._topics[arn]` on every create call. The CFN engine falls back to the create handler when no update handler is registered, so every stack redeploy that touched a topic stack would zero out the topic's `subscriptions`, `messages`, and `tags` lists — turning the topic into a publish-only black hole until each subscriber stack happened to redeploy and re-add itself. The create handler now short-circuits when the topic already exists for the same ARN, updating only the mutable `DisplayName` attribute and leaving the live state intact. Mirrors the same in-place-update pattern already used for ApiGatewayV2 Api / Route / Integration.
- **AWS::ApiGatewayV2::Authorizer is now provisioned by CloudFormation.** This resource type had no CFN handler registered, so any CDK HTTP API that used `HttpUserPoolAuthorizer` or `HttpJwtAuthorizer` would deploy without surfacing an error but with zero authorizers attached — every route on the API ended up effectively public, even ones that templated `AuthorizationType: JWT`. The v2 control-plane handler (`CreateAuthorizer` / `UpdateAuthorizer` / `DeleteAuthorizer`) was already present and working; only the CFN→runtime wiring was missing. The new provisioner translates the CFN PascalCase property shape (`JwtConfiguration: {Audience, Issuer}`) into the camelCase wire format the runtime stores, supports in-place updates so redeploys don't accumulate duplicate authorizer rows, and registers create / update / delete handlers under the same pattern used for ApiGatewayV2 Api / Route / Integration.
- **CloudFormation stack updates no longer leave duplicate Cognito Identity Pools behind.** Same shape of bug as the previously-fixed ApiGatewayV2::Api duplication: every `cdk deploy` against an existing stack containing an `AWS::Cognito::IdentityPool` was minting a fresh identity pool id, leaving the previous pool orphaned in memory. After a few cycles the SDK's `list_identity_pools` returned two rows with identical names, and any client that resolved the pool by name (a common pattern in mobile apps and CDK custom resources) would pick the older, abandoned pool and fail with cryptic "Token is not from a supported provider" errors. The CFN engine falls back to the resource's `create` handler when no `update` handler is registered; `_cognito_identity_pool_update` is now wired up and mutates the existing pool in place under the same physical id.
- **`AWS::Cognito::UserPool` now honors the `UserPoolName` property.** CloudFormation's spec defines the user pool name property as `UserPoolName`, but the provisioner only checked `PoolName` (the boto3 / Cognito-IDP SDK name for the same field). Templates following the AWS CFN docs therefore had their requested name silently dropped, replaced with an auto-generated `<stack>-<logical-id>-<suffix>` value. Anything downstream that referenced the pool by name — custom domains, dependent stacks doing `aws cognito-idp list-user-pools | jq`, or app clients keyed on the configured name — would then break with "pool not found". Both spellings are now accepted, `UserPoolName` first.
- **API Gateway Lambda proxy responses now honor `isBase64Encoded`.** Both the REST (v1) and HTTP API (v2) response shapers ignored the `isBase64Encoded` flag and utf-8-encoded the body verbatim, so a Lambda returning a binary payload (PNG, PDF, protobuf, gzipped JSON, …) had its base64 string sent to the client as text — the browser saw `iVBORw0KGgo...` instead of the actual image bytes. The shaper now base64-decodes the body to raw bytes when the flag is true, matching the alb.py response path and real API Gateway behaviour. Both v1 and v2 are fixed because the bug was an identical six lines duplicated across both files.
- **SQS event source mappings now deliver `messageAttributes` in camelCase to Lambda.** When SQS pushes a message batch to Lambda on real AWS, each record's `messageAttributes` is transformed from the SDK's PascalCase shape (`{StringValue, BinaryValue, DataType, StringListValues, BinaryListValues}`) into a camelCase shape (`{stringValue, binaryValue, dataType, …}`). MiniStack was passing the raw PascalCase shape straight through, so a handler written against the real Lambda SQS event shape — `record["messageAttributes"]["color"]["stringValue"]` — got a `KeyError` locally but worked in production. The poller now normalises the inner attribute keys before assembling the `Records` list; outer record fields, `attributes` (system attrs), and `body` were already correct.
- **CloudFormation stack updates no longer leave duplicate API Gateway v2 resources behind.** When the same CFN stack was deployed twice (the `cdk deploy` pattern), every `AWS::ApiGatewayV2::Api`, `AWS::ApiGatewayV2::Route`, and `AWS::ApiGatewayV2::Integration` resource in the template ended up duplicated on every redeploy — after a few cycles a real-world API could be carrying hundreds of routes with the same route key, each pointing at a different copy of the integration, and the Ref-resolved `ApiId` would change between deploys (breaking downstream resources that cached it). The cause was in the CloudFormation engine, not the API Gateway service itself: ministack's CFN engine falls back to calling the resource's `create` handler when no explicit `update` handler is registered, on the assumption that creates are idempotent. The apigw v2 Api, Route, and Integration provisioners weren't — they minted a fresh `ApiId` / `RouteId` / `IntegrationId` via `new_uuid()` on every call and inserted under that ID, leaving the previous resource in place. All three now have proper `update` handlers that look up the existing resource by the previous physical ID and mutate it in place, so a redeploy mutates the same row instead of leaving a stale copy behind. Replaces the previously-documented `AWS::ApiGatewayV2::Api REPLACED on property changes` quirk.
- **API Gateway v2 now picks the most specific route, matching real AWS.** When an HTTP API had two routes that could both match the same URL — most commonly a literal child path alongside a parametric sibling, like `GET /things/special` next to `GET /things/{id}` — ministack picked whichever route happened to be declared first in the API definition. Real AWS API Gateway v2 picks the most specific route: literal segments outrank `{param}`, which outrank greedy `{param+}`, and earlier segments dominate later ones. The buggy first-match behaviour meant that, depending on CDK template ordering, a request to `/things/special` could be silently routed to the `/things/{id}` handler — the Lambda still ran but received `routeKey: "GET /things/{id}"` with `pathParameters.id = "special"`, breaking downstream handlers that expected the literal route. The matcher now scores every route that matches the request, picks the highest-scoring one, and falls back to `$default` only when nothing else matches. Exact-method routes also win over `ANY` for the same path.
